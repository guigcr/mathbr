...
log_sum_exp(x)
\end{verbatim}
\verb|quantile| takes \verb|p| in $[0,1]$; \verb|percentile| takes \verb|p| in $[0,100]$. Mode counts exact equal values and returns the smallest on a tie. Skewness and excess kurtosis use population central moments without small-sample correction and require positive variance. Correlations require both inputs to vary. The weighted functions require aligned, strictly positive finite weights and divide by the weight sum; weighted variance and covariance have no degrees-of-freedom correction.
\begin{verbatim}
import mathbr
s = mathbr.statistics
...
\end{verbatim}
Binomial assumes independent trials sharing one success probability. Its CDF may require $O(n)$ work, and extreme PMFs can underflow.
\begin{verbatim}
import mathbr
d = mathbr.distributions
